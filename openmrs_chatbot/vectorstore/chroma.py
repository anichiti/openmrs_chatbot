...
class VectorStore:
    ...
    def index_doctor_kb(self):
        try:
            ...
            # Check if collection already has documents - skip PDF loading if it does
            if self.doctor_collection.count() > 0:
                logger.info(f"Doctor KB collection already populated with {self.doctor_collection.count()} documents. Skipping PDF loading.")
                return True
    ...
    def index_patient_kb(self):
        try:
            ...
            # Check if collection already has documents - skip PDF loading if it does
            if self.patient_collection.count() > 0:
                logger.info(f"Patient KB collection already populated with {self.patient_collection.count()} documents. Skipping PDF loading.")
                return True

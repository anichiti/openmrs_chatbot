...
class MedicationResponsePatient:
    ...
    def format_active_medications(medications, patient_name=None):
        ...
        CHANGE 7: Show generic names as "Brand (Generic)" format.
        If generic name not available, shows brand name only.
        ...
        for i, med in enumerate(medications, 1):
            # CHANGE 7: Format drug name with generic name
            drug_name = med.get('drug_name', 'Unknown')
            generic_name = med.get('generic_name') or med.get('generic', None)
            
            # Format as "Brand (Generic)" if generic name exists
            if generic_name and generic_name != drug_name:
                drug_display = f"{drug_name} ({generic_name})"
            else:
                drug_display = drug_name
            
            report += f"[MEDICATION {i}] {drug_display}\n"

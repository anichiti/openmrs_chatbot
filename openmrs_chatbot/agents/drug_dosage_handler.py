...
def extract_drug_name(query: str) -> Optional[str]:
    ...
    # CLINICAL ABBREVIATION EXCLUSION LIST
    # Never treat these as drug names, they are clinical measurements only
    clinical_abbreviations = {
        'spo2', 'bmi', 'bp', 'hr', 'rr', 'ecg', 'cbc', 'wbc', 'rbc', 'hb',
        'o2', 'iv', 'im', 'sc', 'pr', 'od', 'bd', 'tds', 'qds', 'inr', 'crp', 'esr'
    }
    
    query_lower = query.lower()
    
    # PRIORITY: Check for known drug names directly in the query before regex
    known_drugs = [
        'aspirin', 'acetylsalicylic acid', 'ibuprofen', 'paracetamol', 'acetaminophen',
        'amoxicillin', 'cephalexin', 'oxacillin', 'metformin', 'omeprazole',
        'lisinopril', 'amlodipine', 'atorvastatin', 'metoprolol', 'losartan',
        'hydrochlorothiazide', 'gabapentin', 'sertraline', 'fluoxetine',
        'ciprofloxacin', 'azithromycin', 'doxycycline', 'prednisone',
        'naproxen', 'diclofenac', 'morphine', 'codeine', 'tramadol',
    ]
    for known in known_drugs:
        if known in query_lower:
            logger.info(f"[STEP 2] Drug name extracted (known drug match): {known}")
            return known
    ...
    patterns = [
        ...
        # Action patterns BEFORE greedy [WORD] dose pattern
        r'prescribe\s+([a-z]+)',                               # "prescribe aspirin"
        r'(?:take|administer|give)\s+([a-z]+)',               # "take aspirin", "give amoxicillin"
        # Dosage patterns
        ...
        r'(?:using|use|try|consider|start|switch\s+to)\s+([a-z]+)',  # "using cephalexin", "try amoxicillin"
        r'(?:would|could|is)\s+([a-z]+)\s+(?:be|work)',       # "would amoxicillin be appropriate"
        r'(?:appropriate|suitable|safe|good|right)\s+.*?\b([a-z]{4,})\s*(?:for|to|$)',  # "appropriate...amoxicillin for"
        r'(?:how\s+about|what\s+about)\s+([a-z]+)',           # "how about oxacillin"
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            # Avoid extracting common words AND clinical abbreviations
            if drug_name not in ['prescribe', 'dose', 'dosage', 'medication', 'drug', 'take', 'administer', 'give', 'of', 'for', 'effects', 'adverse', 'the', 'are', 'is', 'what', 'main', 'side', 'tell', 'me', 'about', 'info', 'information', 'details', 'profile', 'on', 'this', 'that', 'patient', 'child', 'my', 'to', 'can', 'should', 'safe', 'ok', 'okay', 'it', 'him', 'her', 'them', 'using', 'use', 'try', 'consider', 'start', 'would', 'could', 'thoughts', 'appropriate', 'suitable', 'good', 'right', 'how', 'with', 'much', 'many', 'yes', 'no', 'also', 'some', 'more', 'just', 'only', 'well', 'very', 'really', 'then', 'when', 'will', 'does', 'been', 'need', 'have', 'want', 'like', 'know', 'think', 'make', 'patietn', 'if'] and drug_name not in clinical_abbreviations:
    ...
    # But log it as uncertain - and EXCLUDE clinical abbreviations
    words = query.split()
    for word in words:
        if word[0].isupper() and len(word) > 2 and word not in ['What', 'How', 'The', 'Is', 'Can', 'Should', 'Give', 'Take', 'This', 'That', 'Patient', 'Your', 'Question', 'Drug', 'My', 'Child', 'Would', 'Could', 'Does', 'Will', 'Are', 'Not', 'For', 'About', 'Using', 'Thoughts', 'Any', 'Some', 'And', 'But', 'With', 'From', 'Have', 'Has', 'Had', 'Been', 'Being', 'Was', 'Were']:
            word_lower = word.lower()
            # Skip if it's a clinical abbreviation
            if word_lower in clinical_abbreviations:
                logger.info(f"[STEP 2] Skipping clinical abbreviation: {word}")
                continue

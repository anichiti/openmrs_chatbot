...
from agents.drug_information_fetcher import get_drug_fetcher
from agents.drug_information_response import DrugInformationResponse
...
class ResponseAgent:
    ...
    def _clean_response(self, response_text):
        ...
        # FIX FOR ISSUE 4: Remove bracketed interpretations next to vital values
        text = self._remove_bracketed_interpretations(text)
    ...
    def _remove_bracketed_interpretations(self, text):
        """
        ISSUE 4 FIX: Remove bracketed interpretations from vital values.
        
        Removes patterns like:
        - "96 bpm (normal)" → "96 bpm"
        - "36.5°C (feverish)" → "36.5°C"
        
        Filters these interpretation words:
        normal, abnormal, feverish, elevated, low, high, healthy, concerning, good, bad,
        within range, out of range, above normal, below normal, borderline,
        slightly high, slightly low, dangerously, critically
        """
        import re
        
        # Interpretation words to filter (in brackets/parentheses next to values)
        interpretation_words = [
            'normal', 'abnormal', 'feverish', 'elevated', 'low', 'high', 'healthy',
            'concerning', 'good', 'bad', 'within range', 'out of range', 'above normal',
            'below normal', 'borderline', 'slightly high', 'slightly low', 'dangerously',
            'critically', 'critical', 'severe', 'mild', 'moderate'
        ]
        
        # Build regex pattern to match value followed by bracketed interpretation
        # Pattern: captures numeric value with unit, followed by (interpretation)
        result = text
        for interpretation in interpretation_words:
            # Match: "number unit (interpretation)" or "number unit [interpretation]"
            pattern = r'(\d+(?:\.\d+)?(?:\s*[a-zA-Z°%/]*))\s*\(?(?:' + re.escape(interpretation) + r')\)?'
            # Replace with just the value part
...
"""
    ...
    def generate_drug_information_response(self, drug_name, user_type="DOCTOR", original_question=None):
        """
        Generate targeted drug information response using FDA and RxNorm APIs
        
        Args:
            drug_name: Name of the drug to look up
            user_type: "DOCTOR" or "PATIENT"
            original_question: User's original question for detecting what they asked for
        
        Returns:
            Formatted drug information string (targeted or full)
        """
        try:
            drug_fetcher = get_drug_fetcher()
            drug_info = drug_fetcher.get_comprehensive_drug_info(drug_name)
            
            # Use targeted format that shows only what was asked for
            response = DrugInformationResponse.format_targeted(drug_info, original_question)
            
            return response
        
        except Exception as e:
            logger.error(f"Error generating drug information: {e}")
            return f"Unable to retrieve comprehensive information for '{drug_name}'. Please check your connection and try again."
    
    def generate_drug_interactions_response(self, drug_list, user_type="DOCTOR"):
        """
        ...
        Returns:
            Formatted interactions information
        """
        try:
            drug_fetcher = get_drug_fetcher()
            interactions = drug_fetcher.search_drug_interactions(drug_list)
            return DrugInformationResponse.format_drug_interactions(interactions)
        
        except Exception as e:
            logger.error(f"Error checking drug interactions: {e}")
            return f"Unable to check for interactions. Please try again later."
    
    def detect_drug_query(self, question):
        """
        Detect if the question is asking about drug information
        
        Returns:
            Dict with query type and drug names if detected
        """
        question_lower = question.lower()
        
        # Keywords that indicate drug information query
        drug_info_keywords = [
            'side effect', 'adverse', 'contraindication', 'warning', 'interaction',
            'pregnant', 'nursing', 'breastfeed', 'dosage', 'dose', 'indication',
            'mechanism', 'how does', 'what causes', 'allergy', 'reaction',
            'safe', 'unsafe', 'dangerous', 'harmful', 'benefit', 'risk'
        ]
        
        interaction_keywords = [
            'interact', 'interaction', 'with', 'combined', 'together',
            'both', 'taking', 'and', 'with medication', 'compatibility'
        ]
        
        # Extract drug names (simple heuristic)
        # Looks for capitalized words that might be drug names
        words = question.split()
        
        is_drug_query = any(keyword in question_lower for keyword in drug_info_keywords)
        is_interaction_query = any(keyword in question_lower for keyword in interaction_keywords)
        
        return {
            "is_drug_query": is_drug_query,
            "is_interaction_query": is_interaction_query,
            "question": question
        }

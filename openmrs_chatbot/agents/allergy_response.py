...
class AllergyResponseDoctor:
    ...
    def format_drug_allergy_check(drug_name, allergy_check_result, patient_id, patient_name=None):
        ...
        if allergy_check_result['is_contraindicated']:
            warning_msg = warning_allergy_match(
                {
                    "allergy_type": allergy_check_result['allergen_matched'],
                    "severity": allergy_check_result.get('severity', 'UNKNOWN')
                },
                drug_name,
                role="doctor"
            )
            ...
            if allergy_check_result.get('cross_reactivity'):
                report += f"Cross-Reactivity: {drug_name} belongs to '{allergy_check_result['cross_reactivity']}' class\n"
            if allergy_check_result.get('excipient_warning'):
                report += f"\nExcipient Warning:\n"
                for warn in allergy_check_result['excipient_warning']:
                    report += f"  - {warn}\n"
            ...
            report += f"\n{allergy_check_result['message']}\n"
            if allergy_check_result.get('excipient_warning'):
                report += f"\n[CAUTION] Excipient Warnings:\n"
                for warn in allergy_check_result['excipient_warning']:
                    report += f"  - {warn}\n"
                report += "\nDrug is not directly contraindicated but contains excipients that may trigger non-drug allergies.\n"
                report += "RECOMMENDATION: Assess patient tolerance before prescribing.\n"
        
        report += f"\n{'='*70}\n"
        report += "Clinical Safety Notes:\n"
        report += "  - Always verify with full patient allergy history\n"
        report += "  - Cross-reactivity checked via RxNorm drug classification\n"
        report += "  - Document all allergy considerations in patient record\n"
        report += "  - Counsel patient on signs of allergic reaction\n\n"
        report += "Source: OpenMRS Patient Allergy Record + RxNorm Drug Classification\n"
...
class AllergyResponsePatient:
    ...
    def format_drug_allergy_check(drug_name, allergy_check_result, patient_id, patient_name=None):
        ...
        if allergy_check_result['is_contraindicated']:
            ...
            report += f"Severity: {allergy_check_result['severity']}\n"
            if allergy_check_result.get('cross_reactivity'):
                report += (f"Note: {drug_name} belongs to the same drug family "
                           f"({allergy_check_result['cross_reactivity']}) as your allergen.\n")
            if allergy_check_result.get('excipient_warning'):
                report += f"\nAdditional Warnings:\n"
                for warn in allergy_check_result['excipient_warning']:
                    report += f"  - {warn}\n"
            ...
            report += "[OK] No documented allergy to this medication\n\n"
            report += f"{allergy_check_result['message']}\n\n"
            if allergy_check_result.get('excipient_warning'):
                report += "[NOTICE] Important ingredient information:\n"
                for warn in allergy_check_result['excipient_warning']:
                    report += f"  - {warn}\n"
                report += "\nPlease tell your doctor about your food/environmental allergies\n"
                report += "so they can decide if this medication is safe for you.\n\n"
